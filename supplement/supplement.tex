...
% Science: Careful with the Harris, Kot, and Hedger citations if the
%          style is changed.
...
\newcounter{reference} \setcounter{reference}{67}
...
For the sensitivity analyses on herd size (Fig.~4a, Figure S1), birth
seasonality (Figure S2), and model start time
...
and the proportion of simulations persisting $\unit[10]{y}$
as a function of the focal parameter.

For the parameter sensitivity analysis (Fig.~4b, Figure S3), for each
...
calculated the partial rank correlation coefficient (PRCC)
\autocites{blower_1994}{marino_2008} of the FMDV extinction time to the
model parameters. We used our own PRCC implementation in Python that
uses standard numerical libraries for rank transforming, linear

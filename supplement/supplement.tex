...
% Check the Harris, Kot, and Hedger citations if you change the
% citation style.
...
\begin{figure}
  ...
  \caption{The stable age distribution of the buffalo population
    ...
    hazard is on 1 January ($\tau = \unit[0]{y}$).}
...
The stochastic model was initiated at $t_0 = \unit[0.5]{y}$ (about 1
July), chosen to be some time after the peak of the birth
hazard at $\tau = \unit[0]{y}$ (1 January) so that many susceptible
young buffalo without maternal immunity were available or would soon
be. The model was initiated with 1000 buffalo. The ages of these
buffalo were sampled from the stable age distribution and the sex of
each buffalo was randomly sampled with probability
$p_{\text{female}} = 0.5$ of being female. The buffalo were then
assigned to a random immune state with probabilities depending on
the age of the buffalo, as described below. If there were fewer than 2
susceptible buffalo, the processes of choosing the initial population
was restarted. Otherwise, 2 buffalo were randomly chosen among the
susceptible buffalo with equal probabilities and these were changed to
the infectious immune state.
...
We estimated the hazard of infection using data from
\citeauthor{hedger_1972} \autocite{hedger_1972}.

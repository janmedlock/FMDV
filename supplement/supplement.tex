...
\usepackage[cleanlook]{isodate} % Nature: UK-style dates.
% Nature: Careful with the Harris, Kot, and Hedger citations if the
%         style is changed.
...
% This is appendix #3. The figures that follow are #4, #5, ...
\newcounter{chapter} \setcounter{chapter}{3}
% Nature: Prepend 'S' to the appendix number.
\newcommand{\appendixprefix}{S}
\renewcommand{\thechapter}{\appendixprefix\arabic{chapter}}
...
\begin{figure}
  ...
  \caption{Model diagram. The dashed box and arrows show the
    ...
    chronic transmission that are not present in the model with only
    acute transmission.}
  \label{fig:diagram}
\end{figure}
...
$0$ infected (exposed, infectious, and chronic) buffalo or until
...
chronic immune states and otherwise the newborn has the susceptible
immune state.
...
\begin{figure}
  ...
  \caption{The stable age distribution of the buffalo population
    at $t_0 = \unit[0.5]{y}$, i.e.~about
    {\printyearoff\printdate{2020-07-01}}. The peak in the birth
    hazard is on {\printyearoff\printdate{2020-01-01}}
    ($\tau = \unit[0]{y}$).}
...
The stochastic model was initiated at $t_0 = \unit[0.5]{y}$
(about {\printyearoff\printdate{2020-07-01}}), chosen to be some time
after the peak of the birth hazard at $\tau = \unit[0]{y}$
({\printyearoff\printdate{2020-01-01}}) so that many susceptible young
buffalo without maternal immunity were available or would soon be. The
model was initiated with 1000 buffalo. The ages of these buffalo were
sampled from the stable age distribution and the sex of each buffalo
was randomly sampled with probability $p_{\text{female}} = 0.5$
of being female. The buffalo were then assigned to a random immune
state with probabilities depending on the age of the buffalo, as
described below. If there were fewer than 2 susceptible buffalo, the
processes of choosing the initial population was restarted. Otherwise,
2 buffalo were randomly chosen among the susceptible buffalo with
equal probabilities and these were changed to the infectious immune
state.
...
waning. To be in the chronic state at age $a$, a buffalo must have
been infected at some age $a'$, become chronic with probability
$p_{\text{chronic}}$, and not yet undergone chronic recovery in the
remaining time $a - a'$:
\begin{equation}
  \label{eq:P_C}
  P_{\mathrm{C}}(a) =
  ...
  p_{\text{chronic}}
  S_{\text{chronic recovery}}(a - a') \md a',
\end{equation}
...
(For the model with only acute transmission, $p_{\text{chronic}} = 0$
...
(\autoref{tab:hedger_data}).

Anna classified them. We summed over SAT. There was 1 I that I instead
called a C. I summed the C and R, maybe because it didn't matter for
finding $h_{\mathrm{infection}}$.
...
\begin{table}
  ...
  \begin{tabular}{|l|l|rrrrrr|}
    ...
    & \multicolumn{1}{|c|}{\textbf{Immune state}}
    ...
    & 17 & 0 & 0 & 0 & 0 & 0
    \\
    \raisebox{-0.5em}[0em][0em]{Chronic}
    & Susceptible
    & 1 & 2 & 4 & 2 & 3 & 3
    \\
    & Chronic
    ...
    \\ \hline
  \end{tabular}
  \caption{Counts of buffalo by immune state and age, based on surveys
    of FMDV antibodies \autocite{hedger_1972}.
    The surveyed buffalo were assigned by expert opinion to the immune
    states present in our simplified model with only acute transmission
    (\enquote{Acute}) and to those present in our simplified model
    with both acute and chronic transmission
    (\enquote{Chronic}). These counts were then pooled across
...
\renewcommand{\thefigure}{\appendixprefix\arabic{figure}}
 % Fix some warnings from hyperref.

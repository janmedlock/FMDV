...
def plot_infected(ax, infected, model, SAT, IC, label_model, draft=False):
    ...
    if ax.is_first_row():
        title = f'Initial condition {IC}'
        if label_model:
            model_ = {'chronic': 'carrier'}.get(model, model)
            title = '\n'.join((f'{model_.capitalize()} model',
                               title))
        ...
        for row in range(nrows):
            for (j, model) in enumerate(models):
                ...
                for (k, IC) in enumerate(ICs):
                    col = col_model + k
                    # Models share the x scale.
                    sharex = axes[0, col_model]
